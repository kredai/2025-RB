...
for rep in range(1, num+1):
    src = cwd+run+"/!images/CellField_"+mcs+f"/replicate-{rep:02d}"
    ...
    def image_analyser(image_path):
        ...
        else: major_axis = 0; eccentricity = 0; mean_diameter = 0; rotation = 45
    ...
    ref_file = pathlib.Path(cwd+run+"/"+run+f"-{rep:02d}.dat")
    with open(ref_file, "r") as f:
        ref_rows = [row.strip().split() for row in f.readlines()]

    output_file = cwd+run+f"/!images/"+run+f"-{rep:02d}_"+mcs+".dat"
    with open(output_file, "w", newline="") as out_csv:
        writer = csv.writer(out_csv, delimiter=" ", quotechar="|", quoting=csv.QUOTE_MINIMAL)
        writer.writerow(["Replicate"] + ref_rows[0] + ["NumberOfClusters", "TotalCellArea", "HypoxicFraction", "MajorAxis", "Eccentricity", "Rotation", "MeanDiameter", "Density", "Convolution", "DensityProfile", "CentreDensity", "MarginDensity", "HypoxicDensityProfile"])
        for idx, img in enumerate(images):
            iteration = f"{(int(''.join(filter(str.isdigit, img.stem))) % 10**3):03d}"
            ref_row = next((row for row in ref_rows[1:] if row[0] == iteration), None)
            if ref_row is None: continue 
            num_clusters, cell_area, hypoxic_fraction, major_axis, eccentricity, rotation, mean_diameter, density, convolution, density_profile, cen_density, mar_density, hypoxic_profile = image_analyser(img)
            density_profile = ":".join(f"{d:.4f}" for d in density_profile)
            hypoxic_profile = ":".join(f"{d:.4f}" for d in hypoxic_profile)
            writer.writerow([f"{rep:02d}"] + ref_row + [num_clusters, f"{cell_area:.2f}", f"{hypoxic_fraction:.4f}", f"{major_axis:.2f}", f"{eccentricity:.4f}", f"{rotation:.2f}", f"{mean_diameter:.2f}", f"{density:.4f}", f"{(convolution):.3f}", density_profile, f"{cen_density:.4f}", f"{mar_density:.4f}", hypoxic_profile])

image_dir = pathlib.Path(cwd+run+"/!images")
pattern = f"*{mcs}.dat"
...
for file in files:
    with open(file, "r", newline="") as f:
        ...
        if header is None:
            header = list(file_header) if isinstance(file_header, (list, tuple)) else file_header.split()
        for row in reader:
...
if rows and header:
    output_file = pathlib.Path(cwd+run+f"/{run}_{mcs}.csv")
